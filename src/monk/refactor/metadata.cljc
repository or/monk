...
(defn- combine-metadata-entries-into-map
  [entries]
  (let [new-entries (first (reduce (fn [[result seen-keys] [entry-kind entry-data]]
                                     (let [new-entries (cond
                                                         (ast/is-map? entry-data) (map vec
                                                                                       (partition 2
    ...
    (cond-> []
      (seq deprecated-entries) (conj [:deprecated_metadata_entry
                                      (into [:map]
                                            (apply concat (map second deprecated-entries)))])

      (seq non-deprecated-entries) (conj [:metadata_entry
                                          (into [:map]
                                                (apply concat (map second non-deprecated-entries)))]))))

(defn- dedupe-inline-metadata-entries
  [entries]
  (sort (first (reduce (fn [[result seen-keys]
                            [entry-kind key
                             :as entry]]
